function [audio_out,Hz] = FIR_hp(audio_in,n_audio_out,fs,fp,As)
...
% As = atenuación  en la banda de rechazo en dB
  ...
  OmegaP = 2*pi*fp; OmegaS = 2*pi*fs;    % Frecuencia en rad/seg
  wp = OmegaP/fms; ws = OmegaS/fms;      % Frecuencia digital en rad
  delta_w = wp - ws;                     % Ancho banda de transición [rad]
  ...
  hd = ideal_hp(wc,M);                     % Generar filtro pasa Altos
  ...
  % ********************* Impresión de Datos ******************************
  fprintf('\n* Datos de %s utilizando Filtro FIR Pasa-Alto',audio_in);
