function [audio_out,Hz] = FIR_stp(audio_in,n_audio_out,fp1,fs1,fs2,fp2,As)
% Diseño y aplicación de filtro Rechaza-Banda FIR a partir de un audio
% -----------------------------------------
...
% As = atenuación  en la banda de rechazo en dB
  ...
  delta_wLP = ws1 - wp1;                 % banda de transición LP[rad]
  delta_wHP = wp2 - ws2;                 % banda de transición HP [rad]
  ...
  hd = ideal_lp(wc1,M)+ideal_lp(pi,M)-ideal_lp(wc2,M); % Filtro Rechaza B.
  Hz = filt(hd,1,Ts)                       % F. Transferencia Digital RSTP
  audio_out = filter(hd,1,x);% Aplicación de filtro a entrada
  audiowrite(n_audio_out,audio_out,fms);   % Escritura de archivo de salida
  [db,mag,fase,grd,w] = freqz_m(cell2mat(Hz.num),1);
  % ********************* Impresión de Datos ******************************
  fprintf('\n* Datos de %s utilizando Filtro FIR Rechaza-Banda',audio_in);
  ...
  axis([0 5000 -0.1 1.1]); title('Magnitud en valor absoluto');
